...
export class DatabaseStorage implements IStorage {
  ...
  async getAllEducation(): Promise<Education[]> {
    const educationList = await db.select().from(education).orderBy(education.sortOrder);
  ...
  async getAllCaseStudies(): Promise<CaseStudy[]> {
    const caseStudiesList = await db.select().from(caseStudies).orderBy(desc(caseStudies.createdAt));
